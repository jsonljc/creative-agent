...
| Q10 | **18th pinned PCD constant `PCD_DISCLOSURE_RESOLVER_VERSION = "pcd-disclosure-resolver@1.0.0"`. Literal appears in exactly ONE non-test source file** (`disclosure-resolver-version.ts`). All consumers import the symbol. | Matches SP13's actual single-source pinning rule (`sp13-anti-patterns.test.ts:61` asserts the resolver source contains zero `"pcd-selector@"` literals — only the version-constant file holds it). The SP13 design doc's "literal in two files" wording was tightened in implementation; SP14 codifies the single-source rule explicitly. Resolver-logic version is distinct from registry row's per-tuple `version: int` — conflating would be a category error. |
...
**Migration timestamp is intentional.** The `20260514150000_pcd_disclosure_template_sp14` folder name is the spec's prescribed timestamp matching the spec date (2026-05-14). Implementer agents must NOT "correct" it to the actual wall-clock time at implementation; the timestamp's role is to lock the migration's lexicographic order against SP12 (`20260514120000_pcd_creator_identity_license_sp12`) and any future SP-numbered migration. Treat the literal as part of the spec.
...
- **U6: Is `text.max(2000)` enough headroom for real legal copy?** v1 placeholder seed strings are well under 200 chars; the 2000-char ceiling is generous for typical platform disclosure snippets but **may pinch when legal authors multi-paragraph platform-specific text** (e.g. a SG HSA + MY MCMC bilingual combined disclosure for slimming, were that vertical to be re-added). Not a blocker for SP14; flagged for the legal-authoring CLI design (deferred slice). If a real authored row would exceed 2000 chars, the constraint widens via a one-line zod edit + an `ALTER TABLE ... ALTER COLUMN text TYPE text` migration; the resolver itself is text-length-agnostic.
